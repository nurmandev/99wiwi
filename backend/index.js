...
app.use(function (req, res, next) {
  if (
    proxy_data.ip_online.find((_) => _.ip_address === req.socket.remoteAddress)
  ) {
    const proxy_found = proxy_data.ip_online.find(
      (_) => _.ip_address === req.socket.remoteAddress
    ).proxy;

    if (
      !proxy_data.error_proxies.find((_) => _ === proxy_found.proxy_address)
    ) {
      next();
      return;
    }

    proxy_data.proxy_using.splice(
      proxy_data.proxy_using.findIndex((_) => _ === proxy_found.proxy_address),
      1
    );
    proxy_data.ip_online.splice(
      proxy_data.ip_online.findIndex(
        (_) => _.ip_address === req.socket.remoteAddress
      ),
      1
    );
  ...
  // find proxy
  const proxy_found = proxy_list.find(
    (_) =>
      proxy_data.proxy_using.indexOf(_.proxy_address) === -1 &&
      proxy_data.error_proxies.indexOf(_.proxy_address) === -1
  );
